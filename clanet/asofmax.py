import math

import torch
import torch.nn as nn
from torch.nn import Parameter


def myphi(x, m):
    x = x * m
    return (
        1
        - x**2 / math.factorial(2)
        + x**4 / math.factorial(4)
        - x**6 / math.factorial(6)
        + x**8 / math.factorial(8)
        - x**9 / math.factorial(9)
    )


class AngleLinear(nn.Module):
    """A-Softmax classifier used by the original CLA-NET implementation."""

    def __init__(self, in_features, out_features, m=4, phiflag=True, bias=False):
        super().__init__()
        if bias:
            raise ValueError("AngleLinear follows the original CLA-NET bias=False setup.")
        self.in_features = in_features
        self.out_features = out_features
        self.weight = Parameter(torch.Tensor(in_features, out_features))
        self.weight.data.uniform_(-1, 1).renorm_(2, 1, 1e-5).mul_(1e5)
        self.phiflag = phiflag
        self.m = m
        self.mlambda = [
            lambda x: x**0,
            lambda x: x**1,
            lambda x: 2 * x**2 - 1,
            lambda x: 4 * x**3 - 3 * x,
            lambda x: 8 * x**4 - 8 * x**2 + 1,
            lambda x: 16 * x**5 - 20 * x**3 + 5 * x,
        ]

    def forward(self, input):
        x = input
        w = self.weight

        ww = w.renorm(2, 1, 1e-5).mul(1e5)
        xlen = x.pow(2).sum(1).pow(0.5)
        wlen = ww.pow(2).sum(0).pow(0.5)

        cos_theta = x.mm(ww)
        cos_theta = cos_theta / xlen.view(-1, 1) / wlen.view(1, -1)
        cos_theta = cos_theta.clamp(-1, 1)

        if self.phiflag:
            cos_m_theta = self.mlambda[self.m](cos_theta)
            theta = cos_theta.detach().acos()
            k = (self.m * theta / math.pi).floor()
            n_one = k * 0.0 - 1
            phi_theta = (n_one**k) * cos_m_theta - 2 * k
        else:
            theta = cos_theta.acos()
            phi_theta = myphi(theta, self.m).clamp(-1 * self.m, 1)

        cos_theta = cos_theta * xlen.view(-1, 1)
        phi_theta = phi_theta * xlen.view(-1, 1)
        return cos_theta, phi_theta
